...
namespace EVERFI.Foundry.Classes {
  ...
  public class UserType {
    ...
    internal UserType(Types MyType, Roles MyRole) {
      Type = MyType;
      Role = MyRole;
      UserRole = (UserRole)int.Parse(((int)MyRole).ToString() + ((int)MyType).ToString());
    }

    public UserType(UserRole MyRole) {
      UserRole = MyRole;
      Type = (Types)((int)MyRole % 10);
      Role = (Roles)((int)MyRole / 10);
    }
    ...
    internal static Types StringToType(string value) {
      ...
      switch (value) {
        ...
        case "Adult Financial Manager":
          return Types.AdultFinancialManager;
        case "Adult Financial Learner":
          return Types.AdultFinancialLearner;
        default:
